---
title: Music Circle Underground Market
type: entity
visibility: player-safe
status: active
first_session: 2026-07-02
last_updated_session: 2026-07-02
canon_status: canon
confidence: high
tags: [location, nashville, underground, squatter-market, data-broker]
sources:
  - ../Sessions/2026-07-02.md
---

# Music Circle Underground Market

## Overview

A tunnel-connected squatter market below the city, reached during the tail from **Music Circle** to **Palermo's** hideout.

## Known Facts

- The area sits in an underground tunnel system with side passages, blocked controllable doors, storm drains, grates, and diffuse light from above.
- It is dirty, cramped, and populated enough that disguises and local etiquette matter more than simple stealth.
- **Kurgan** remembered the area from old Humanis-linked business where higher-ups dealt with Palermo or his circles.
- Palermo used a central protected structure in the area as an operating location.
- Several apparent drainage or runoff holes concealed power and Matrix utility connections feeding that central structure.
- Local residents avoided the central structure, suggesting Palermo's area was recognized as dangerous or off-limits.
- **Devil rats** live in the water near the approach and are tolerated unusually close to metahuman habitation.
- Rat wranglers are apparently paid to keep the devil rats around as living guard animals that scare off outsiders.

## Relationships

- Linked to **[Palermo](../NPCs/Palermo.md)** as a compromised operating location.
- Linked to **[Music Circle](Music-Circle.md)** through the surface-level surveillance and tail route.
- Linked to **[Griswell Data Services](../Organizations/Griswell-Data-Services.md)** only indirectly through the Alvin/Palermo data-leak job.

## Relevant Sessions

- **2026-07-02** — first explored during the conclusion of the Griswell surveillance run.

## Open Questions

- Who controls the market, if anyone?
- How formal is the rat-wrangler arrangement?
- What other shadow operators use the underground space?
- Did Palermo permanently abandon the location after the crew exposed it?

## Sources

- [Session 2026-07-02](../Sessions/2026-07-02.md)
